...
<head>
    ...
    <title>{{ \App\Helpers\NecHelper::setting_get('site_name', 'NEC South Sudan') }} - Forgot Password</title>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .brand-icon-circle img { width: 100%; height: 100%; object-fit: cover; border-radius: 50%; position: relative; z-index: 1; }
        .brand-icon-circle::before, .brand-icon-circle::after {
            content: '';
            position: absolute;
            border-radius: 50%;
        ...
        .brand-sub { color: rgba(255,255,255,.5); font-size: .85rem; max-width: 260px; line-height: 1.5; margin: 0 auto 24px; }
        .brand-features { display: flex; flex-wrap: wrap; gap: 10px; justify-content: flex-start; max-width: 280px; margin-right: auto; }
        .brand-feat {
            display: flex; align-items: center; gap: 6px; padding: 7px 14px;
            background: rgba(255,255,255,.04); border: 1px solid rgba(255,255,255,.06);
            border-radius: 20px; font-weight: 500; font-size: .78rem; color: rgba(255,255,255,.6);
        }
        .login-form-wrap { width: 62%; padding: 40px 48px; display: flex; flex-direction: column; justify-content: center; }
        ...
        .input-icon { position: absolute; left: 14px; top: 50%; transform: translateY(-50%); color: #94a3b8; font-size: .9rem; z-index: 3; }
        .input-wrap .form-control {
            height: 50px; padding-left: 44px; padding-right: 14px; border-radius: 10px;
            border: 2px solid #e2e8f0; font-size: .9rem; font-weight: 500;
            transition: all .2s; background: #f8fafc; font-family: 'Urbanist', sans-serif;
        }
        .input-wrap .form-control:focus { border-color: var(--nec-primary); box-shadow: 0 0 0 3px rgba(252,96,17,.08); background: #fff; }
        .input-wrap .form-control:disabled { opacity: .6; background: #f1f5f9; }
        .btn-jibu {
            position: relative; overflow: hidden; width: 100%; padding: .75rem 2rem;
            font-weight: 700; border-radius: 50px; border: none;
            background: linear-gradient(135deg, #FC6011, #FF8F40); color: #fff;
            font-size: .95rem; transition: all .3s; box-shadow: 0 4px 15px rgba(252,96,17,.25);
            cursor: pointer; font-family: 'Urbanist', sans-serif;
        }
        .btn-jibu:hover { transform: translateY(-1px); box-shadow: 0 6px 20px rgba(252,96,17,.35); }
        .btn-jibu:disabled { opacity: .6; cursor: not-allowed; transform: none; }
        .btn-shimmer { position: absolute; top: 0; left: -100%; width: 100%; height: 100%; background: linear-gradient(90deg,transparent,rgba(255,255,255,.15),transparent); animation: shimmer 2.5s infinite; }
        @keyframes shimmer { 0% { left: -100%; } 100% { left: 200%; } }
        .alert-custom { border-radius: 10px; padding: 10px 14px; font-size: .85rem; display: flex; align-items: center; gap: 8px; }
        .back-link { color: #94a3b8; font-size: .83rem; text-decoration: none; display: inline-flex; align-items: center; gap: 4px; }
        .back-link:hover { color: var(--nec-primary); }
        .divider { display: flex; align-items: center; gap: 10px; color: #94a3b8; font-size: .8rem; font-weight: 500; margin: 20px 0; }
        .divider::before, .divider::after { content: ''; flex: 1; height: 1px; background: #e2e8f0; }
        .trust-badges { display: flex; justify-content: center; gap: 8px; flex-wrap: nowrap; padding-top: 10px; }
        .trust-badge { display: inline-flex; align-items: center; gap: 5px; font-size: .75rem; font-weight: 600; color: #4A4B4D; padding: 5px 12px; border-radius: 20px; background: #f1f5f9; border: 1px solid #e2e8f0; white-space: nowrap; }
        .trust-badge i { font-size: .7rem; }
        @media (max-width: 767.98px) {
            .login-wrapper { flex-direction: column; max-width: 420px; }
            .login-brand { width: 100%; padding: 30px 25px; }
                ...
                <form method="POST" action="{{ route('forgot-password') }}">
                    ...
                    <div class="field-group" style="margin-bottom:18px;">
                        <label class="form-label" style="display:flex;align-items:center;gap:6px;font-size:.82rem;font-weight:600;color:#1f2937;margin-bottom:6px;">
                            <span style="display:inline-flex;align-items:center;justify-content:center;width:22px;height:22px;border-radius:6px;font-size:.6rem;background:#dbeafe;color:#3b82f6;"><i class="fas fa-envelope"></i></span>
                            Email Address
                        </label>
                        <div class="input-wrap">
                            <i class="fas fa-envelope input-icon"></i>
                            <input type="email" name="email" class="form-control" placeholder="Admin email address" value="{{ old('email', $email ?? '') }}" {{ isset($otpSent) ? 'disabled' : '' }} required autofocus>
                        </div>
                    ...
                    <button type="submit" class="btn-jibu">
                        ...
                        {{ isset($otpSent) ? 'Resend Reset Code' : 'Send Reset Code' }}
                    </button>
                </form>
