...
<div class="card border-0 shadow-sm rounded-3 mb-4" style="background:#f8fafc;border:1px solid #e9edf2;">
    <div class="card-body">
        <div class="d-flex align-items-center gap-2 mb-3">
            ...
            @if(request('search'))
                <a href="{{ route('admin.education.index', $currentStatus ? ['status' => $currentStatus] : []) }}" class="text-decoration-none" style="font-size:0.75rem;color:#64748b;margin-left:auto;">Clear</a>
            @endif
        </div>
        <form action="" method="GET" class="row g-3">
            ...
            <div class="col-md-4">
                ...
                <input type="text" name="search" class="form-control" placeholder="Search materials..." value="{{ request('search') }}" style="border-radius:8px;">
            </div>
...
<div class="card border-0 shadow-sm rounded-3 overflow-hidden mb-3">
        ...
        <table class="table align-middle mb-0">
            <thead>
                <tr>
                    ...
                    <th style="background:#2E8B57;color:#fff;font-weight:600;border-bottom:2px solid #1f6b3f;padding:10px 12px;font-size:0.75rem;letter-spacing:0.3px;text-transform:uppercase;">Language</th>
                    <th style="background:#2E8B57;color:#fff;font-weight:600;border-bottom:2px solid #1f6b3f;padding:10px 12px;font-size:0.75rem;letter-spacing:0.3px;text-transform:uppercase;">Audience</th>
            ...
            <tbody>
                ...
                <tr style="border-bottom:1px solid #f1f3f5;">
                    ...
                    <td style="padding:10px 12px;">
                        @if($can('education.update'))
                        <a href="{{ route('admin.education.edit', $item->id) }}" class="fw-semibold text-decoration-none" style="color:#1e293b;">{{ $item->title }}</a>
                        @else
                    ...
                    <td style="padding:10px 12px;"><span class="badge bg-info">{{ ucfirst(str_replace('_', ' ', $item->content_type ?? 'document')) }}</span></td>
                    <td style="padding:10px 12px;color:#475569;">{{ $item->language ?? 'English' }}</td>
                    <td style="padding:10px 12px;color:#475569;">{{ $item->target_audience ?? 'general' }}</td>
                    ...
                    <td style="padding:10px 16px 10px 12px;white-space:nowrap;text-align:right;">
                        ...
                        @if($can('education.update'))
                        <a href="{{ route('admin.education.edit', $item->id) }}" class="btn btn-sm rounded-3" style="padding:3px 8px;background:rgba(59,130,246,0.08);color:#3b82f6;border:none;" title="Edit"><i class="fas fa-edit"></i></a>
                        @endif
                        ...
                        @if($can('education.delete'))
                        <button class="btn btn-sm rounded-3" style="padding:3px 8px;background:rgba(239,68,68,0.08);color:#ef4444;border:none;" onclick="confirmDelete('{{ route('admin.education.destroy', $item->id) }}')" title="Delete"><i class="fas fa-trash"></i></button>
                        @endif
                        @else
                        @if($can('education.update'))
                        <a href="{{ route('admin.education.restore', $item->id) }}" class="btn btn-sm rounded-3" style="padding:3px 8px;background:rgba(46,139,87,0.08);color:#2E8B57;border:none;" title="Restore"><i class="fas fa-undo"></i></a>
                        @endif
                        @if($can('education.delete'))
                        <button class="btn btn-sm rounded-3" style="padding:3px 8px;background:rgba(239,68,68,0.08);color:#ef4444;border:none;" onclick="confirmDelete('{{ route('admin.education.force-delete', $item->id) }}')" title="Delete"><i class="fas fa-times"></i></button>
                        @endif
                        @endif
                    </td>
                </tr>
                ...
                <tr>
                    <td colspan="8" class="text-center py-5">
                        <div style="width:52px;height:52px;border-radius:14px;background:#f1f5f9;display:flex;align-items:center;justify-content:center;margin:0 auto 12px;">
                            <i class="fas fa-book" style="color:#94a3b8;font-size:20px;"></i>
                        </div>
                        <p style="color:#64748b;margin-bottom:8px;font-size:0.9rem;">No education materials found.</p>
                    </td>
                </tr>
                @endforelse
            </tbody>
    ...
    <div class="card-footer bg-white border-top d-flex flex-wrap justify-content-between align-items-center gap-2 px-4 py-3">
        <div style="font-size:0.75rem;color:#64748b;">Showing {{ $materials->firstItem() }} to {{ $materials->lastItem() }} of {{ $materials->total() }} entries</div>
        {{ $materials->links() }}
    </div>
    @endif
</div>
