...
<div id="wp-editor">
    <div id="wp-admin-bar">
        <div class="wp-bar-left">
            ...
            <h1>Edit Event</h1>
            <span class="wp-status-badge {{ $event->status }}">{{ $event->status }}</span>
    ...
    <form id="event-form" action="{{ route('admin.events.update', $event->id) }}" method="POST" enctype="multipart/form-data">
        @csrf @method('PUT')
        ...
        <div id="wp-editor-grid">
            <div id="wp-editor-main">
                <div id="titlediv">
                    <input type="text" name="title" id="title" value="{{ old('title', $event->title) }}" placeholder="Add title" autocomplete="off" oninput="autoSlug()" required>
                </div>

                <div id="edit-slug-box">
                    <strong>Permalink:</strong>
                    <span class="slug-prefix">{{ url('/events') }}/</span>
                    <input type="text" name="slug" id="slug" value="{{ old('slug', $event->slug) }}" placeholder="auto-generated" class="slug-field">
                ...
                <div id="wp-content-editor">
                    <textarea name="description" id="editor" class="form-control" rows="12">{{ old('description', $event->description) }}</textarea>
                </div>
            </div>
            ...
            <div id="wp-editor-sidebar">
                <div class="meta-box">
                    ...
                    <div class="meta-box-body">
                        ...
                        <div class="meta-field">
                            ...
                            <input type="text" name="organizer" class="form-control form-control-sm" value="{{ old('organizer', $event->organizer) }}">
                        </div>
                    </div>
                </div>
                    ...
                    <div class="meta-box-body">
                        <div class="meta-field">
                            ...
                            <select name="event_type" class="form-select form-select-sm">
                                <option value="public" @selected(($event->event_type ?? 'public') === 'public')>Public</option>
                        ...
                        <div class="meta-field">
                            ...
                            <input type="text" name="location" class="form-control form-control-sm" value="{{ old('location', $event->location) }}" placeholder="Venue or virtual link">
                        </div>
                        <div class="meta-field">
                            <label>Start Date &amp; Time <span class="text-danger">*</span></label>
                            <input type="datetime-local" name="start_date" class="form-control form-control-sm @error('start_date') is-invalid @enderror" value="{{ old('start_date', $event->start_date ? \Carbon\Carbon::parse($event->start_date)->format('Y-m-d\TH:i') : '') }}" required>
                            @error('start_date')<div class="invalid-feedback">{{ $message }}</div>@enderror
                        </div>
                        <div class="meta-field">
                            <label>End Date &amp; Time</label>
                            <input type="datetime-local" name="end_date" class="form-control form-control-sm" value="{{ old('end_date', $event->end_date ? \Carbon\Carbon::parse($event->end_date)->format('Y-m-d\TH:i') : '') }}">
                        </div>
                    </div>
                </div>
                ...
                <div class="meta-box">
                    ...
                    <div class="meta-box-body text-center">
                        <div id="featured-image-preview" class="mb-2" @if(!$event->featured_image)style="display:none;"@endif>
                            <img id="image-preview-img" src="{{ $event->featured_image }}" alt="" style="max-width:100%;max-height:160px;border-radius:6px;">
                        ...
                        <div id="featured-image-placeholder" class="wp-image-placeholder" @if($event->featured_image)style="display:none;"@endif onclick="document.getElementById('image-input').click()">
                ...
                <div class="meta-box">
                    ...
                    <div class="meta-box-body">
                        <div class="meta-field">
                            ...
                            <textarea name="meta_description" class="form-control form-control-sm" rows="3" maxlength="500" placeholder="Brief description for search engines...">{{ old('meta_description', $event->meta_description) }}</textarea>
                        </div>
                    </div>
                </div>
            </div>
...
function previewFeaturedImage(input) {
    if (input.files && input.files[0]) {
        ...
        reader.onload = function(e) {
            ...
            document.getElementById('remove-image').value = '';
        };
