    ...
    <tbody>
        ...
        <tr>
            ...
            <td class="text-center" style="white-space:nowrap;">
                ...
                <a href="{{ route('admin.events.edit', $item->id) }}" class="btn btn-sm btn-outline-primary" title="Edit"><i class="fas fa-edit"></i></a>
                <a href="{{ route('admin.events.toggle-status', $item->id) }}" class="btn btn-sm btn-outline-{{ $item->status === 'published' ? 'warning' : 'success' }}" title="{{ $item->status === 'published' ? 'Unpublish' : 'Publish' }}"><i class="fas fa-{{ $item->status === 'published' ? 'eye-slash' : 'eye' }}"></i></a>
                <button class="btn btn-sm btn-outline-danger" onclick="confirmDelete('{{ route('admin.events.destroy', $item->id) }}')" title="Delete"><i class="fas fa-trash"></i></button>
                @else
                <a href="{{ route('admin.events.restore', $item->id) }}" class="btn btn-sm btn-outline-success" title="Restore"><i class="fas fa-undo"></i></a>
                <button class="btn btn-sm btn-outline-danger" onclick="confirmDelete('{{ route('admin.events.force-delete', $item->id) }}')" title="Delete"><i class="fas fa-times"></i></button>
                @endif
            </td>
        </tr>
        @empty
