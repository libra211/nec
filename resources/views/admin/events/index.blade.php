...
<div class="d-flex justify-content-between align-items-center mb-3">
    ...
    <a href="{{ route('admin.events.create') }}" class="btn btn-primary"><i class="fas fa-plus me-1"></i> Add Event</a>
</div>
...
<div class="card border-0 shadow-sm rounded-3 mb-4" style="background:#f8fafc;border:1px solid #e9edf2;">
    <div class="card-body">
        <div class="d-flex align-items-center gap-2 mb-3">
            ...
            @if(request('search'))
                <a href="{{ route('admin.events.index', $currentStatus ? ['status' => $currentStatus] : []) }}" class="text-decoration-none" style="font-size:0.75rem;color:#64748b;margin-left:auto;">Clear</a>
            @endif
        </div>
        <form action="" method="GET" class="row g-3">
            ...
            <div class="col-md-4">
                ...
                <input type="text" name="search" class="form-control" placeholder="Search events..." value="{{ request('search') }}" style="border-radius:8px;">
            </div>
...
<div class="card border-0 shadow-sm rounded-3 overflow-hidden mb-3">
        ...
        <table class="table align-middle mb-0">
            <thead>
                <tr>
                    ...
                    <th style="background:#2E8B57;color:#fff;font-weight:600;border-bottom:2px solid #1f6b3f;padding:10px 12px;font-size:0.75rem;letter-spacing:0.3px;text-transform:uppercase;">Location</th>
                    ...
                    <th style="background:#2E8B57;color:#fff;font-weight:600;border-bottom:2px solid #1f6b3f;padding:10px 12px;font-size:0.75rem;letter-spacing:0.3px;text-transform:uppercase;">Start Date</th>
            ...
            <tbody>
                ...
                <tr style="border-bottom:1px solid #f1f3f5;">
                    ...
                    <td style="padding:10px 12px;">
                        <a href="{{ route('admin.events.edit', $item->id) }}" class="fw-semibold text-decoration-none" style="color:#1e293b;">{{ e($item->title) }}</a>
                    </td>
                    <td style="padding:10px 12px;"><span class="badge bg-info">{{ e($item->event_type ?? 'public') }}</span></td>
                    <td style="padding:10px 12px;color:#475569;">{{ e($item->location ?? '—') }}</td>
                    ...
                    <td style="padding:10px 12px;color:#64748b;">{{ \Carbon\Carbon::parse($item->start_date)->format('d M Y') }}</td>
                    <td style="padding:10px 16px 10px 12px;white-space:nowrap;text-align:right;">
                        @if($item->status !== 'trash')
                        <a href="{{ route('admin.events.edit', $item->id) }}" class="btn btn-sm rounded-3" style="padding:3px 8px;background:rgba(59,130,246,0.08);color:#3b82f6;border:none;" title="Edit"><i class="fas fa-edit"></i></a>
                        <a href="{{ route('admin.events.toggle-status', $item->id) }}" class="btn btn-sm rounded-3" style="padding:3px 8px;background:rgba({{ $item->status === 'published' ? '6,182,212' : '46,139,87' }},0.08);color:{{ $item->status === 'published' ? '#0891b2' : '#2E8B57' }};border:none;" title="{{ $item->status === 'published' ? 'Unpublish' : 'Publish' }}"><i class="fas fa-{{ $item->status === 'published' ? 'eye-slash' : 'eye' }}"></i></a>
                        <button class="btn btn-sm rounded-3" style="padding:3px 8px;background:rgba(239,68,68,0.08);color:#ef4444;border:none;" onclick="confirmDelete('{{ route('admin.events.destroy', $item->id) }}')" title="Delete"><i class="fas fa-trash"></i></button>
                        @else
                        <a href="{{ route('admin.events.restore', $item->id) }}" class="btn btn-sm rounded-3" style="padding:3px 8px;background:rgba(46,139,87,0.08);color:#2E8B57;border:none;" title="Restore"><i class="fas fa-undo"></i></a>
                        <button class="btn btn-sm rounded-3" style="padding:3px 8px;background:rgba(239,68,68,0.08);color:#ef4444;border:none;" onclick="confirmDelete('{{ route('admin.events.force-delete', $item->id) }}')" title="Delete"><i class="fas fa-times"></i></button>
                        @endif
                    </td>
                </tr>
                @empty
                <tr>
                    <td colspan="8" class="text-center py-5">
                        <div style="width:52px;height:52px;border-radius:14px;background:#f1f5f9;display:flex;align-items:center;justify-content:center;margin:0 auto 12px;">
                            <i class="fas fa-calendar-times" style="color:#94a3b8;font-size:20px;"></i>
                        </div>
                        <p style="color:#64748b;margin-bottom:8px;font-size:0.9rem;">No events found.</p>
                    </td>
                </tr>
                @endforelse
            </tbody>
    ...
    <div class="card-footer bg-white border-top d-flex flex-wrap justify-content-between align-items-center gap-2 px-4 py-3">
        <div style="font-size:0.75rem;color:#64748b;">Showing {{ $events->firstItem() }} to {{ $events->lastItem() }} of {{ $events->total() }} entries</div>
        {{ $events->links() }}
    </div>
    @endif
</div>
