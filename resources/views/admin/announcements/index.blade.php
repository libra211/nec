...
<div class="d-flex justify-content-between align-items-center mb-3">
    ...
    <a href="{{ route('admin.announcements.create') }}" class="btn btn-nec-green"><i class="fas fa-plus me-1"></i> Add Announcement</a>
</div>
        ...
        <tr>
            ...
            <td class="text-center">
                ...
                <div class="btn-group btn-group-sm">
                    <a href="{{ route('admin.announcements.edit', $item->id) }}" class="btn btn-outline-primary"><i class="fas fa-edit"></i></a>
                    <a href="{{ route('admin.announcements.toggle-status', $item->id) }}" class="btn btn-outline-{{ $item->status === 'published' ? 'warning' : 'success' }}"><i class="fas fa-{{ $item->status === 'published' ? 'eye-slash' : 'eye' }}"></i></a>
                    <button class="btn btn-outline-danger" onclick="confirmDelete('{{ route('admin.announcements.destroy', $item->id) }}')"><i class="fas fa-trash"></i></button>
                </div>
                @else
                <div class="btn-group btn-group-sm">
                    <a href="{{ route('admin.announcements.restore', $item->id) }}" class="btn btn-outline-success"><i class="fas fa-undo"></i></a>
                    <button class="btn btn-outline-danger" onclick="confirmDelete('{{ route('admin.announcements.force-delete', $item->id) }}')"><i class="fas fa-times"></i></button>
                </div>
                @endif
            </td>
        </tr>
