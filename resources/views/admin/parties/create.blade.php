...
.form-control-color { padding: 0.25rem; cursor: pointer; }
.color-swatch {
  width: 36px; height: 36px; border-radius: 50%; cursor: pointer;
  border: 2.5px solid transparent; transition: var(--transition); flex-shrink: 0;
}
...
.preview-avatar {
  ...
  font-weight: 700; font-size: 1.2rem; flex-shrink: 0; transition: var(--transition);
}
...
<div class="d-flex justify-content-between align-items-center mb-4 animate-in">
    <div>
        <h2 class="mb-1" style="font-weight:700;"><i class="fas fa-flag text-primary me-2"></i>Add Political Party</h2>
        <p class="text-muted mb-0 small">Register a new political party for the 2026 elections</p>
    </div>
...
<form action="{{ route('admin.parties.store') }}" method="POST" enctype="multipart/form-data" id="partyForm">
@csrf
<div class="row g-4">
    {{-- LEFT COLUMN --}}
    <div class="col-lg-8">
            ...
            <div class="card-body p-4">
                <div class="row g-4">
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="text" name="acronym" id="fieldAcronym" class="form-control @error('acronym') is-invalid @enderror" value="{{ old('acronym') }}" required maxlength="20" oninput="updatePreview();countChar(this,'acroCount')">
                        <div class="char-count" id="acroCount">0/20</div>
                        @error('acronym') <div class="invalid-feedback">{{ $message }}</div> @enderror
                    </div>
                    <div class="col-md-6">
                        <label class="form-label">Leader / Chairperson</label>
                        <input type="text" name="leader" id="fieldLeader" class="form-control @error('leader') is-invalid @enderror" value="{{ old('leader') }}" maxlength="255" oninput="updatePreview()">
                        @error('leader') <div class="invalid-feedback">{{ $message }}</div> @enderror
                    </div>
                    <div class="col-md-6">
                        <label class="form-label">Year Founded</label>
                        <input type="number" name="founded" class="form-control @error('founded') is-invalid @enderror" value="{{ old('founded') }}" min="1900" max="{{ date('Y') }}" placeholder="e.g. 2001">
                        @error('founded') <div class="invalid-feedback">{{ $message }}</div> @enderror
                    </div>
                    <div class="col-12">
                        <label class="form-label">Party Color</label>
                        <div class="d-flex align-items-center gap-3 flex-wrap">
                            <input type="color" name="color" id="fieldColor" class="form-control form-control-color @error('color') is-invalid @enderror" value="{{ old('color', '#2E8B57') }}" style="height:44px;width:60px;padding:3px;" oninput="updatePreview()">
                            <div class="d-flex gap-1 flex-wrap" id="colorPalette">
                                ...
                                <div class="color-swatch {{ old('color') === $c ? 'active' : '' }}" style="background:{{ $c }};" data-color="{{ $c }}" onclick="pickColor(this,'{{ $c }}')"></div>
                                @endforeach
                            </div>
                        </div>
    ...
    <div class="col-lg-4">
        ...
        <div class="card border-0 shadow-sm mb-4 animate-in animate-in-d2" style="border-radius:var(--card-radius);">
            ...
            <div class="card-body p-3">
                <div class="preview-card border" id="previewCard" style="background:#2E8B5708;">
                    <div class="p-3">
                        <div class="d-flex align-items-center gap-3">
                            <div class="preview-avatar" id="previewAvatar" style="background:#2E8B57;color:#fff;">PT</div>
                            <div>
                                <h6 class="fw-bold mb-0" id="previewName" style="font-size:0.95rem;">Party Name</h6>
                                <span class="badge mt-1" id="previewBadge" style="background:#2E8B5720;color:#2E8B57;font-size:0.6rem;">ACR</span>
                            </div>
                        </div>
                        <div class="mt-2 small text-muted" id="previewLeader"><i class="fas fa-user-tie me-1" style="font-size:0.65rem;"></i>No leader set</div>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="card-body p-4">
                <div class="d-flex align-items-center justify-content-between">
                    ...
                    <div class="form-check form-switch mb-0">
                        <input class="form-check-input" type="checkbox" role="switch" id="statusToggle" style="width:2.5rem;height:1.25rem;cursor:pointer;" checked onchange="document.getElementById('statusField').value = this.checked ? '1' : '0'">
                    </div>
                </div>
        ...
        <div class="card border-0 shadow-sm mb-4 animate-in animate-in-d3" style="border-radius:var(--card-radius);">
            ...
            <div class="card-body p-4">
                ...
                <div class="alert alert-info py-2 small mb-0 mt-3" style="border-radius:8px;font-size:0.75rem;">
...
function updatePreview() {
    ...
    document.getElementById('previewBadge').textContent = acro || 'ACR';
