...
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <h2 class="mb-1"><i class="fas fa-flag text-primary me-2"></i>Edit Political Party</h2>
        <p class="text-muted mb-0 small">Update party details and registration documents</p>
    </div>
            ...
            <div class="card-body">
                <div class="row g-3">
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="text" name="acronym" class="form-control @error('acronym') is-invalid @enderror" value="{{ old('acronym', $party->acronym) }}" required>
                        @error('acronym') <div class="invalid-feedback">{{ $message }}</div> @enderror
                    </div>
                    <div class="col-md-6">
                        <label class="form-label fw-semibold">Leader</label>
                        <input type="text" name="leader" class="form-control @error('leader') is-invalid @enderror" value="{{ old('leader', $party->leader) }}">
                        @error('leader') <div class="invalid-feedback">{{ $message }}</div> @enderror
                    </div>
                    <div class="col-md-6">
                        <label class="form-label fw-semibold">Year Founded</label>
                        <input type="number" name="founded" class="form-control @error('founded') is-invalid @enderror" value="{{ old('founded', $party->founded) }}" min="1900" max="{{ date('Y') }}">
                    ...
                    <div class="col-md-6">
                        ...
                        @if($party->logo)
                            <div class="mt-2"><img src="{{ asset('storage/' . $party->logo) }}" alt="" width="60" class="rounded border"></div>
                        @endif
                        @error('logo') <div class="invalid-feedback">{{ $message }}</div> @enderror
                    </div>
                    <div class="col-md-6">
                        <label class="form-label fw-semibold">Party Color</label>
                        <input type="color" name="color" class="form-control form-control-color @error('color') is-invalid @enderror" value="{{ old('color', $party->color ?? '#000000') }}" style="height:44px;">
        ...
        <div class="card border-0 shadow-sm">
            ...
            <div class="card-body">
                ...
                <div class="d-flex align-items-center gap-2 p-2 rounded border bg-light">
                    ...
                    <span class="small flex-fill">{{ basename($party->registration_document) }}</span>
                    <a href="{{ asset('storage/' . $party->registration_document) }}" target="_blank" class="btn btn-sm btn-outline-primary" style="padding:2px 8px;font-size:11px;"><i class="fas fa-eye"></i></a>
                </div>
                @endif
...
<div class="mt-4">
    <button type="submit" class="btn btn-nec-green"><i class="fas fa-save me-1"></i> Update Party</button>
</div>
</form>
@endsection
