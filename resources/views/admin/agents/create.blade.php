@extends('admin.layouts.app', ['title' => 'Add Registration Agent'])

@section('content')
<div class="d-flex justify-content-between align-items-center mb-4">
    <h2 class="mb-0">Add Registration Agent</h2>
...
<form method="POST" action="{{ route('admin.agents.store') }}">
        ...
        <div class="card-body">
            <div class="row g-3">
                    ...
                    <select name="state" class="form-select @error('state') is-invalid @enderror">
                        ...
                        @foreach(config('nec.regions') as $region)
                            ...
                            @foreach($region['admin_areas'] ?? [] as $area)
                                <option value="{{ $area }}" {{ old('state') === $area ? 'selected' : '' }}>{{ $area }}</option>
                            @endforeach
                        @endforeach
                    </select>
                    @error('state')
                ...
                <div class="col-md-4">
                    ...
                    <input type="text" name="county" class="form-control @error('county') is-invalid @enderror" value="{{ old('county') }}">
                    @error('county')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                <div class="col-md-4">
                    ...
                    <input type="text" name="constituency" class="form-control @error('constituency') is-invalid @enderror" value="{{ old('constituency') }}">
                    @error('constituency')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                <div class="col-md-6">
                    ...
                    <input type="text" name="payam" class="form-control @error('payam') is-invalid @enderror" value="{{ old('payam') }}">
                    @error('payam')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                <div class="col-md-6">
                    ...
                    <input type="text" name="boma" class="form-control @error('boma') is-invalid @enderror" value="{{ old('boma') }}">
                    @error('boma')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
            ...
            <div class="row g-3">
                <div class="col-md-4">
                    ...
                    <select name="assigned_state" class="form-select @error('assigned_state') is-invalid @enderror">
                        ...
                        @foreach(config('nec.regions') as $region)
                            @foreach($region['states'] as $state)
                                <option value="{{ $state }}" {{ old('assigned_state') === $state ? 'selected' : '' }}>{{ $state }}</option>
                            @endforeach
                            @foreach($region['admin_areas'] ?? [] as $area)
                                <option value="{{ $area }}" {{ old('assigned_state') === $area ? 'selected' : '' }}>{{ $area }}</option>
                            @endforeach
                        @endforeach
                    </select>
                ...
                <div class="col-md-4">
                    ...
                    <input type="text" name="assigned_county" class="form-control @error('assigned_county') is-invalid @enderror" value="{{ old('assigned_county') }}">
                ...
                <div class="col-md-4">
                    ...
                    <input type="text" name="assigned_constituency" class="form-control @error('assigned_constituency') is-invalid @enderror" value="{{ old('assigned_constituency') }}">
    ...
    <div class="card border-0 shadow-sm mb-4">
        ...
        <div class="card-body">
            <textarea name="notes" class="form-control @error('notes') is-invalid @enderror" rows="3" placeholder="Additional notes about this agent...">{{ old('notes') }}</textarea>
            @error('notes')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
